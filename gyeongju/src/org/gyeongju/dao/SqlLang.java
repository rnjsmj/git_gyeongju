...
public interface SqlLang {
	...
	//여기어때
	String SELECT_ALL_PLACE = "select * from place order by pno desc";
	String SELECT_PLACE_BYNO = "select * from place where pno=?";
	String SELECT_PLACE_CULTURAL = "select * from place where ptype='문화재' order by pno desc";
	String SELECT_PLACE_THEME = "select * from place where ptype='테마파크' order by pno desc";
	String SELECT_PLACE_BEACH = "select * from place where ptype='해변' order by pno desc";
