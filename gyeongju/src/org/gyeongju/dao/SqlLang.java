...
public interface SqlLang {
	...
	String SELECT_PLACE_THEME = "select * from ( select rownum as rnum, place.* from place where ptype='테마파크/공원' order by pno desc ) where rnum between ? and ?";
	String SELECT_PLACE_ETC = "select * from ( select rownum as rnum, place.* from place where ptype='기타' order by pno desc ) where rnum between ? and ?";
